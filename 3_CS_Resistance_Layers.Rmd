...
# canopy_rsmpl <- resample(canopy_mask,mt_nlcd)
...
roads_shp <- readOGR("../R_output_files/transpo/roads_fltr_shp.shp") %>%
  st_as_sf() %>%
  mutate(res_values = c(500,250,1000,750)) %>%
  as(.,"Spatial") %>%
...
writeRaster(roads_ras_f,"../R_output_files/circuitscape/resistance_variables/roads_res.tif")
...
writeRaster(rivers_ras, "../R_output_files/circuitscape/resistance_variables/rivers_res.tif")
...
max_res_trans <- raster.transformation(max_resistance,trans = "stretch",smin = 0.1,smax = 1000)

# Canopy Cover  
canopy_res <- raster("../R_output_files/circuitscape/resistance_variables/canopy_cover.tif") 
max_canopy <- cellStats(x = canopy_res,stat = max)
canopy_res <- canopy_res/max_canopy

# Slope 
slope_res = raster("../R_output_files/circuitscape/resistance_variables/mt_slope_r.tif")
max_slope <- cellStats(x = slope_res,stat = max)
slope_res = slope_res/max_slope

max_res_stack_adj <- max_res_trans*(1-slope_res)*(1+canopy_res)

resistance_final <- mask(max_res_stack_adj,mt_nlcd)
NAvalue(resistance_final) <- -9999
...
writeRaster(resistance_resample,"../R_output_files/circuitscape/resistance/resistance_FINAL.asc", format = "ascii",overwrite=T)
```
