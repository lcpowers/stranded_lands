...
for(i in 1:length(county_names)){
  ...
  # county_i_sections = read_sf(paste0("../R_output_files/marxan_finalPU_shp/",county_name,"/",county_name,"_finalPU.shp")) %>%
  #   filter(status == 1)
  ...
  nlcd_mask = raster::crop(nlcd,extent(county_i_sections)) %>% mask(.,county_i_sections)
...
for(i in 1:length(county_names)){
  ...
  # crop species raster stack to current county extent then mask to stranded planning units
  ...
  species_df = data.frame(name = names(species_mask),
                            total = cellStats(species_mask,sum))
...
for(i in 1:length(NLCDbyCounty_list)){
  ...
  ## Write output
  write_delim(spec, path = paste0("../R_output_files/MarxanInput_byCounty/",county_names[i],"/spec_crop.dat"), delim =" ")
