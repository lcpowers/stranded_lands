...
## Does it make sense to have non-stranded public land from this file locked into the reserve system? If there is a lot of grazing happening on the land, does it make more sense to center around the core areas that are actually wildlife habitat?

## Grazing Allotments
# mt_grazing = read_sf("../R_input_files/mt_pad/PAD_combined_clean.shp") %>% 
#   st_transform(.,crs(nlcd)) %>% 
#   filter(str_detect(Loc_Ds,"Grazing")) %>% 
...
for(i in 1:length(county_names)){
  ...
  core_areas = read_sf("../R_output_files/circuitscape/core_area_shapefiles/",county_name,"/",county_name,"_coreareas_final.shp") %>% 
    dplyr::select() %>% 
  ...
  write_delim(final_pu,paste0("../R_output_files/MarxanInput_byCounty/",county_name[i],"/pu.dat"),delim = " ")
