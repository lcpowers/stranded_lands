...
# county_names = as.vector(counties$names)
...
for(i in 1:length(county_names)){
  ...
  county_i_PUs = read_sf(paste0("../R_output_files/marxan_finalPU_shp/",county_name,"/",county_name,"_finalPU.shp"))
  ...
  for(j in PU_ids){
    ...
    output_df_nlcd_merge = merge(output_df_nlcd,county_i_features)
...
for(i in 1:length(county_names)){
  ...
  for(j in PU_ids){
    ...
    species_df = data.frame(species = names(species_section_mask),
                            amount = cellStats(species_section_mask,sum)) %>% 
      filter(amount>0)
